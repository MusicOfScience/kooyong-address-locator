...
Designed for Google Colab usage where you provide the RSS XML (or path to a
file that contains it). The scraper:
...
def extract_pdf_url(html: str, page_url: str) -> Optional[str]:
    ...
    def is_pdf_candidate(href: str, text: str) -> bool:
        if any(pattern.search(href) for pattern in PDF_HREF_PATTERNS):
            return True
        return "pdf" in text.lower()

    for anchor in soup.find_all("a", href=True):
        href = anchor["href"]
        text = anchor.get_text(strip=True)
        if is_pdf_candidate(href, text):
...
def scrape_pdfs(
    ...
    for idx, item in enumerate(items, start=1):
        ...
        item_response = _request_with_retries(session, item.link)
